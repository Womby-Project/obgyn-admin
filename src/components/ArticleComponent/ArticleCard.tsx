...
export interface Article {
  ...
  imageUrl: string; // maps to thumbnail_url
  views: number; // maps to views_count
  author?: string; // For "Posted By You" & "All Articles"
  source?: string; // For external (non-OBGYN) items
  publishedDate?: string; // formatted published_at
  archivedDate?: string; // formatted (from updated_at or a date you store)
}
...
export default function ArticleCard({ article, variant, onViewArticle }: ArticleCardProps) {
  return (
    <div
      className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden flex flex-col h-full hover:shadow-lg transition-shadow duration-300 cursor-pointer"
      onClick={() => onViewArticle(article)}
    >
      <img
        src={article.imageUrl || "https://placehold.co/600x400/E9AEA4/FFFFFF?text=Wombly"}
        alt={article.title}
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="text-xs text-gray-500 flex flex-wrap items-center gap-x-4 gap-y-1 mb-3">
          {(variant === 'all' || variant === 'posted') && article.author && (
            <div className="flex items-center gap-1.5">
              <UserIcon /><span>By: {article.author}</span>
            </div>
          )}
          {variant === 'all' && article.source && (
            <div className="flex items-center gap-1.5">
              <SourceIcon /><span>{article.source}</span>
            </div>
          )}
          {(variant === 'all' || variant === 'posted') && article.publishedDate && (
            <div className="flex items-center gap-1.5">
              <CalendarIcon /><span>{`Published: ${article.publishedDate}`}</span>
            </div>
          )}
          {variant === 'archived' && article.archivedDate && (
            <div className="flex items-center gap-1.5">
              <CalendarIcon /><span>{`Archived: ${article.archivedDate}`}</span>
            </div>
          )}
        </div>
        <p className="text-sm text-gray-600 mb-4 flex-grow line-clamp-3">{article.description}</p>
        <div className="text-xs text-gray-500 flex items-center gap-1.5">
          <ViewsIcon /><span>{article.views} Views</span>
        </div>
      </div>
      {variant === 'posted' && (
        <div className="border-t border-gray-200 bg-gray-50 p-3 text-center">
          <button
            className="flex items-center justify-center w-full gap-2 text-sm font-medium text-gray-600 hover:text-red-500 transition-colors"
